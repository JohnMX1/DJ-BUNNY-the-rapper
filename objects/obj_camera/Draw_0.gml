...
if obj_player.boost = true {
	matrix_set(matrix_world, matrix_build(obj_player.x, obj_player.y, (obj_player.z-130)+((obj_player.image_scale+2)*2), obj_player.wii*5, 180, dir, 80, 80, 120));
	vertex_submit(vb_boost, pr_trianglelist, sprite_get_texture(spr_boost, obj_player.wii));
	matrix_set(matrix_world, matrix_build_identity());
}
if obj_player.drift = true {
	if obj_player.hinputi = -1 {
		spr = spr_shadow;
	}
	else if obj_player.hinputi = 1 {
		spr = spr_shadow_flip;
	}
	sus = obj_player.player_direction;
}
else {
	if sus > 0 {
		sus -= 1;
...
if keyboard_check(ord("Z")) and obj_player.drift = false and obj_player.jump = false or keyboard_check(ord("X")) or obj_player.drift = true {
	matrix_set(matrix_world, matrix_build(obj_smoke1.x, obj_smoke1.y, -3, 270, obj_player.zprev, obj_player.look_dir, 40, 40, 80));
	vertex_submit(vb_player_alt, pr_trianglelist, sprite_get_texture(obj_smoke1.sprite_index, obj_smoke1.image_index));
	matrix_set(matrix_world, matrix_build_identity());
	
	matrix_set(matrix_world, matrix_build(obj_smoke2.x, obj_smoke2.y, -3, 270, obj_player.zprev, obj_player.look_dir, 40, 40, 80));
	vertex_submit(vb_player_alt, pr_trianglelist, sprite_get_texture(obj_smoke2.sprite_index, obj_smoke2.image_index));
	matrix_set(matrix_world, matrix_build_identity());
	
	matrix_set(matrix_world, matrix_build(obj_smoke3.x, obj_smoke3.y, -3, 270, obj_player.zprev, obj_player.look_dir, 40, 40, 80));
	vertex_submit(vb_player_alt, pr_trianglelist, sprite_get_texture(obj_smoke3.sprite_index, obj_smoke3.image_index));
	matrix_set(matrix_world, matrix_build_identity());
	
	matrix_set(matrix_world, matrix_build(obj_smoke4.x, obj_smoke4.y, -3, 270, obj_player.zprev, obj_player.look_dir, 40, 40, 80));
